...
### 新增功能
- **🎨 圖片編輯器改進**:
  - **自動關閉**: 切換左側檔案時自動關閉編輯器，避免混淆
  - **Prompt 歷史記錄**: 
    - 自動記錄最近使用的 3 個 prompt
    - 5 個永久保存槽位（左鍵保存/套用，右鍵刪除）
  - **響應式畫布**: Canvas 隨視窗大小變化自動調整尺寸
  - **按鈕位置優化**: 另存/覆蓋/取消按鈕向上移動約 100px

- **🤖 AI 分析結果清除按鈕**:
  - 預覽面板 AI 結果區塊新增 ✕ 清除按鈕
  - 點擊可同時清除記憶體和磁碟快取中的 AI 分析結果

- **🚫 AI 分析跳過不支援檔案類型**:
  - 批量 AI 分析時自動跳過壓縮檔、CAD、3D模型、材質、字體等不支援的檔案類型
  - 預覽面板對這些檔案類型隱藏 AI 分析按鈕

### 修復
- **🌐 i18n 翻譯重複鍵問題**:
  - 修復三個語言檔案中 `imageEditor` 區塊重複導致翻譯失敗的問題

## [2.11.0] - 2026-01-02
### 新增功能
- **🔗 關聯檔名分析**:
  - 選取 2 個以上有 AI 分析結果的檔案，點擊 🔗 按鈕開啟分析對話框
  - AI 自動判斷檔案關係類型：
    - **series（連續系列）**: 同類型連續檔案，使用 `主題_001.ext`, `主題_002.ext` 格式
    - **related（相關檔案）**: 同內容不同格式（如 PSD 及其導出檔），使用**同名不加序號**
    - **independent（獨立檔案）**: 保留各自建議名稱
  - 統一命名建議，確保所有相關檔案使用一致的 base name
  - **雙擊編輯**: 雙擊任一檔案的建議名稱可編輯 base name，所有檔案同步更新
  - **順序調整**: 點擊 ▲/▼ 按鈕調整檔案順序，序號自動重新編排
  - **選擇性套用**: 點擊勾選框可選擇性地採用或排除特定建議
  - 支援 Prompt 設定自訂：Settings > Prompt 設定 > 關聯檔名分析

### 技術改進
- 新增 `LinkedAnalysisDialog` 組件（React + Framer Motion）
- 新增 `ai:analyzeLinkedFiles` IPC handler
- 新增 `analyzeLinkedFiles` 類型定義到 `electron.d.ts`
- 新增 i18n 翻譯（zh-TW, en, ja）
- 擴展 `loadPromptConfig` 支援 `linkedAnalysis` 類型
## [2.10.0] - 2026-01-01
### 新增功能
- **✂️ 圖片編輯器裁切工具**:
  - 拖曳選擇裁切區域，可調整大小
...
### 修復
- **🖼️ 圖片編輯器調整效果截斷問題**:
  - 從 Fabric.js `applyFilters()` 改為 CSS `filter` 屬性
  - 調整亮度/對比度/飽和度時圖片不再被截斷
  - 保存時使用 offscreen canvas 來 bake CSS filter 效果

- **🎵 拆音檔後刷新檔案列表**:
  - 視頻檔案提取音檔成功後，左側檔案列表會自動刷新

- **📋 圖片編輯器翻譯完整化**:
  - 補齊所有 imageEditor 相關翻譯（zh-TW, en, ja）
  - 包含標題、選擇、畫筆、螢光筆、箭頭、圓形、矩形、亮度、對比度、飽和度等

- **🎯 畫布初始化改進**:
  - 使用延遲初始化確保容器已完全渲染
  - 設定畫布大小範圍限制（寬度 400-2000px，高度 300-1500px）

## [2.9.0] - 2026-01-01
### 新增功能
- **🎙️ 音頻逐字稿完整實現**:
  - 點擊「取得逐字稿」按鈕，使用 OpenRouter API 將音頻轉為文字
  - 逐字稿自動快取到 `.autonova` 資料夾，避免重複呼叫 API
  - 選擇音頻檔案時自動載入已快取的逐字稿
  - 點擊「取得逐字稿」按鈕會強制重新轉錄並覆蓋快取
  - 支援 MP3、WAV、FLAC、OGG、M4A、AAC 格式
  - 使用 OpenRouter 的 `input_audio` 格式發送音頻資料

- **🎵 音頻 AI 分析**:
  - 有逐字稿的音頻檔案可進行 AI 分析
  - 沒有逐字稿時，AI 按鈕會顯示「請先取得逐字稿」
  - AI 分析使用 `audioAnalysis` 專用 Prompt 設定
  - 返回建議檔名、關鍵字和描述
...
### 修復
- **九宮格位置固定問題**:
  - 修復刪除資料夾後其他資料夾會「遞補」的問題
  - 移除所有 `slot ?? (index + 1)` 的 fallback 邏輯
  - 確保每個資料夾都有明確的 slot 值
  - 載入 Preset 時自動 migrate 缺少 slot 的資料夾

### 技術改進
- 新增 `audioTranscriptStates` 狀態追蹤（按檔案路徑）
- 新增 `handleTranscribeAudio` 非同步回調
- 新增 `getCachedTranscript` IPC handler
- 新增 `DEFAULT_AUDIO_ANALYSIS_PROMPT` 預設提示詞
- `transcribeAudio` 新增 `forceRefresh` 參數
- 修正 OpenRouter 音頻 API 格式：`text` 在前，`input_audio` 使用 snake_case

## [2.8.0] - 2025-12-31
### 新增功能
- **📝 AI 提示詞生成功能**:
  - 圖片編輯器新增「提示詞生成」按鈕（青綠色）
  - 分析當前圖片（包含任何編輯/標記），自動生成可用於 AI 生圖的詳細提示詞
  - 使用文字/視覺模型分析圖片內容、風格、構圖、光線、氛圍等
  - 生成的提示詞自動填入 AI 編輯輸入框，方便直接使用

- **🎭 AI 遮罩修圖功能**:
  - 圖片編輯器新增「AI 遮罩修圖」按鈕（琥珀色）
  - 在圖片上使用畫筆、矩形、圓形等工具繪製彩色遮罩
  - 輸入描述指定各遮罩區域要替換的內容（如「黑色區域放桌子，紅色區域放貓」）
  - 發送原圖+遮罩圖+提示詞給 AI，實現局部精準修改

- **🎨 Prompt 設定擴展**:
  - 「AI 圖片生成」分頁重新設計，現包含三個獨立模型配置：
    - ✨ AI 生成模型（原有功能）
    - 📝 提示詞生成模型（使用視覺模型）
    - 🎭 AI 遮罩修圖模型（使用生圖模型）
  - 可分別自訂各功能的 System Prompt

### 技術改進
- 擴展 `PromptSettingsDialog.tsx`，新增 `promptGen` 和 `maskInpaint` 配置結構
- 擴展 `ImageEditor.tsx`，新增 `handlePromptGen` 和 `handleMaskInpaint` 函數
- 新增 `ai:generatePrompt` 和 `ai:maskInpaint` IPC handlers
- 擴展 `imageOperations.ts` 模組，新增後端處理函數
- `main.ts` 新增模型和 System Prompt 獲取函數

- **📜 檔案列表虛擬滾動**:
  - 偏好設定新增「檔案虛擬載入下限」選項
  - 當搜尋結果超過設定閾值（預設 300）時，自動啟用虛擬滾動
  - 大幅提升大資料夾的效能，避免卡頓或崩潰
  - 虛擬滾動只渲染可見區域的項目，記憶體使用固定
  - 使用 `react-window` 庫實現高效能滾動

- **📷 RAW 相機檔案預覽**:
  - 支援多種相機 RAW 格式：CR2、CR3、NEF、ARW、DNG、ORF、RW2、PEF、RAF 等
  - 優先提取內嵌 JPEG 縮圖（毫秒級速度）
  - 無內嵌縮圖時自動進行 RAW 解碼
  - 顯示相機元數據：相機型號、解析度、ISO、光圈、快門速度
  - 預覽結果自動快取，下次載入更快
  - 使用 `dcraw-vendored-win32` 處理 RAW 檔案

- **🎵 音頻分析功能**（基礎架構）:
  - Prompt 設定中新增「音頻逐字稿」和「音頻分析」兩個分頁
  - 音頻逐字稿：使用 AI 語音辨識模型（如 Whisper）將音頻轉為文字
  - 音頻分析：根據逐字稿內容分析音頻並生成檔名和描述
  - 支援的音頻格式：MP3、WAV、FLAC、OGG、M4A、AAC、WMA
  - 預覽面板新增「取得逐字稿」按鈕（音頻檔案時顯示）

- **🎬 影片拆音檔功能**:
  - 影片預覽時新增「拆音檔」按鈕（AI 分析按鈕左側）
  - 一鍵將影片音軌提取為同名 MP3 檔案
  - 使用 FFmpeg 進行高品質音頻轉換（192kbps, 44.1kHz）
  - 輸出檔案自動保存到影片所在目錄
...
### 新增功能
- **📖 OCR 文字辨識功能**:
  - 新增 OCR 按鈕於預覽面板右上角（圖片、PDF 檔案可用）
  - 支援圖片 OCR：一鍵辨識圖片中的所有文字
  - 支援 PDF OCR：逐頁辨識，顯示進度條
  - PDF 超過 10 頁時可選擇頁碼範圍辨識
  - 辨識結果可一鍵複製到剪貼簿
  - 💾 「存成文字」功能：保存為同名 `.txt` 檔案（UTF-8 編碼）
  - 保存後自動刷新左側檔案列表，立即看到新生成的文字檔
  - 保存成功後對話框自動關閉

- **🔧 OCR Prompt 設定**:
  - 在 Prompt 設定對話框新增「OCR 文字辨識」分頁
  - 可自訂 OCR 使用的 System Prompt 和 User Prompt
  - 可選擇 OCR 專用的 AI 視覺模型
  - OCR 使用低溫度 (0.1) 以確保辨識準確性

### 技術改進
- 新增 `electron/modules/ocr.ts` 模組處理 OCR 邏輯
- 新增 `src/components/OCRDialog.tsx` 對話框組件
- 擴展 `pdfPreview.ts` 模組導出 `getPdfPageCount` 和 `getPdfPageAsBase64` 函數
- PDF 頁面使用 2.0 倍縮放渲染以提高 OCR 準確度
- 新增 `fs:writeTextFile` IPC handler 供 OCR 保存文字檔使用
...
- **📁 檔案衝突處理**:
  ...
  - 跳過：跳過已存在的檔案，不進行任何操作
  - 覆蓋（檔案）：直接覆蓋目標路徑的同名檔案
  - 覆蓋（資料夾）：Smart Merge，遞迴合併資料夾內容

- **🔍 搜索偏好完整生效**:
  - 修復搜索使用硬編碼值的問題
  - 現在正確使用偏好設定中的 maxDirs、maxFiles、maxDepth、skipDirs

- **📄 PDF 快取優化**:
  - AI 批量分析 PDF 時，若開啟 PDF 快取模式，自動保存預覽圖到快取
  - 下次打開 PDF 時直接使用快取，無需重新渲染

- **📊 PPT/PPTX 預覽支援**:
  - 新增 PowerPoint 檔案預覽功能
  - 優先使用 LibreOffice 轉換為 PDF 後渲染（顯示 3 頁：首頁、中間、末頁）
  - 若無 LibreOffice，回退使用 PPTX 內嵌縮略圖
  - 預覽結果自動快取

- **📝 Word 預覽條文編號**:
  - DOCX 轉換為 HTML 保留列表編號和格式
  - 自動編號（1. 2. 3.）現在正確顯示
  - 支援粗體、斜體、表格等格式
...
- **🎨 Adobe Illustrator (.ai) 預覽**:
  - 新增 .ai 檔案預覽功能
  - 複用 PDF 渲染邏輯（AI 檔案內嵌 PDF 格式）
  - 支援多頁預覽

- **🎮 3D 模型預覽 (Three.js)**:
  - 新增 GLB、GLTF、FBX、OBJ、STL 格式預覽
  - 軌道控制（滑鼠旋轉、縮放、平移）
  - 自動適應模型大小和相機位置
  - 顯示頂點數、面數、動畫數
  - 線框模式、自動旋轉、重置視角按鈕
  - 獨立 ThreeDPreview 組件，模組化設計

- **🖼️ 紋理預覽 (Three.js Loaders)**:
  - 新增 DDS、KTX2、EXR、HDR 格式預覽
  - 顯示解析度、格式、Mipmap 數量
  - 縮放、Alpha 通道顯示、平鋪預覽功能
  - 獨立 TexturePreview 組件，模組化設計

- **🎨 主題色切換 (Accent Color Theming)**:
  - 5 種強調色可選：藍色、粉色、綠色、黃色、灰色
  - 偏好設定中即時切換，立即生效
  - CSS 變量架構，156 處顏色引用自動適應
  - 設定自動保存，下次啟動保持選擇
...
- **網路磁碟刪除失敗**:
  - 修復在網路磁碟（NAS、SMB 共享）上刪除檔案失敗的問題
  - 添加回退機制：若回收站失敗，嘗試直接刪除
